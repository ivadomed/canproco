...
def analyze_prediction(prediction, lesion_mask, animaPath, output_folder):
    ...
    os.system(f'sct_maths -i {prediction} -bin 0.1 -o {lesion_mask_pred_bin_path}')
    ...
    # if the ground truth is empty we skip the analysis
    if np.sum(nib.load(lesion_mask_bin_path).get_fdata()) == 0:
        print(f'{name} has an empty lesion mask, skipping analysis')
        subject_results['empty_mask'] = True
        return subject_results

    # else we continue the analysis
    subject_results['empty_mask'] = False
