...
Example of run:
    $ python nnunet/seg_sc_and_convert_bids_to_nnunet.py --path-data /path/to/data_extracted --path-out /path/to/nnUNet_raw --taskname TASK-NAME --tasknumber DATASET-ID --contrasts PSIR --qc-folder /path/to/qc_folder
...
Arguments:
    ...
    --qc-folder : Path to the qc_folder
...
def main():
    ...
    for image_file in image_files:
        ...
        if (file_id in str(labelled_imgs)):
            ...
            label_file_data = nib.load(label_file)
            label_file_data = np.asarray(label_file_data.dataobj)
            label_file_data = np.where(label_file_data > label_threshold, 1, 0)
            
            multi_label_slice = np.zeros(label_file_data.shape)
            multi_label_slice[sc_seg==1] = 1
            multi_label_slice[label_file_data==1] = 2
