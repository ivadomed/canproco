...
def main():
    ...
    for task, files in dict_yml.items():
        for file in files:
            ...
            # Copy image
            copy_file(os.path.join(args.path_in, utils.get_subject(file), utils.get_ses(file), utils.get_contrast(file), file),
                      os.path.join(path_tmp, utils.get_subject(file), utils.get_ses(file), utils.get_contrast(file)))
            # Copy label if exists
            if suffix_label is not None:
                copy_file(os.path.join(args.path_in, utils.get_subject(file), utils.get_ses(file), utils.get_contrast(file),
