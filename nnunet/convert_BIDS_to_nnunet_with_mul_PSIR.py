...
This script was adapted from the script convert_BIDS_to_nnunet.py from the nnunet repository but we added the extract step to multiply the image by -1 if it is in the PSIR contrast.

Example of run:
    $ python convert_BIDS_to_nnunet_with_mul_PSIR.py --path-data /path/to/data_extracted --path-out /path/to/nnUNet_raw --taskname TASK-NAME --tasknumber DATASET-ID  --contrasts PSIR,STIR --test-ratio 0.2 --time-point ses-M0 --type training --exclude-file /path/to/exclude_file.yml
