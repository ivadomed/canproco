...
Example:
    python convert_nnunet_to_BIDS.py --pred-folder /path/to/predictions --out-folder /path/to/output/folder --conversion-dict /path/to/conversion/dict
...
def main():
    ...
    for pred in pred_files:
        ...
        for files in conversion_dict:
            ...
            if ((pred_name.split('.')[0] in conversion_dict[files].split("/")[-1]) and ('imagesTs' in conversion_dict[files]) and ('0000' in conversion_dict[files]) ) or (pred_name.split('.')[0] in conversion_dict[files].split("/")[-1] and args.not_imageTs and ('0000' in conversion_dict[files])) :
                ...
                # and the new name
                new_name_lesion = subject_full.split('.')[0] + '_pred.nii.gz'
                ...
                os.system(f"sct_maths -i {pred} -bin 0.1 -o {os.path.join(output_subject_path, new_name_lesion)}")
