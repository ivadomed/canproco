...
This script prepares the data for the training of a region based nnU-Net model taking two channels as input (i) STIR and invertes PSIR, and (ii)  SC mask and returning a mask with SC=1 and lesion=2.
...
def build_dataset_for_training(args):
    ...
    for image_file in image_files:
        ...
        if count_below_limit :
            ...
            channel_1 = nib.load(image_file_nnunet_channel_1)
            img = nib.load(sc_mask_file)
            #change the header and replace it by the header of channel 1
            final_img = nib.Nifti1Image(img.get_fdata(), channel_1.affine, channel_1.header)

            nib.save(final_img, image_file_nnunet_channel_2)
            ...
            create_multi_label_mask(lesion_mask_file, sc_mask_file, disc_level_file, label_file_nnunet)
